...
namespace PedestrianSimulation.Cameras
{
    ...
    /// Sets the cameras 
    /// </summary>
    [AddComponentMenu("Simulation/Camera/2D/Revit Level Camera Controller"), DisallowMultipleComponent]
    public class Camera2DRevitLevelController : MonoBehaviour
    {
        ...
        private static void SetFloorView(int floor)
        {
            ...
            Debug.Assert(environment != null, $"{typeof(Camera2DRevitLevelController)} could not find GameObject with tag \"{EnvironmentTag}\"");
            foreach (SpeckleProperties d in environment.GetComponentsInChildren<SpeckleProperties>(true))
            {
